...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (empty($_REQUEST['full_name']) || empty($_REQUEST['email']) || empty($_REQUEST['message'])) {
        ...
        try {
            ...
            $mail->Username = 'ngondtps36014@fpt.edu.vn'; // SMTP username
            $mail->Password = 'nbet wrhq xmax xvkh'; // SMTP password
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS; // Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` also accepted
            $mail->Port = 587; // TCP port to connect to

            //Recipients
            $mail->setFrom('ngondtps36014@fpt.edu.vn', 'Tech Device');
            $mail->addAddress('diepngon182004@gmail.com', 'Tech Infor');
            // $mail->addAddress('ellen@example.com'); 
            // $mail->addReplyTo('info@example.com', 'Information');
            $mail->addCC('ngondtps36014@fpt.edu.vn');
